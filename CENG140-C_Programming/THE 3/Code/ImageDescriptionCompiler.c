...
Boolean saveImage ( Image image , char * fileName )
{
  ...
  Pppmfile = fopen(fileName,"w");

  fprintf(Pppmfile, "P3\n"                                 );
  fprintf(Pppmfile, "# %s\n",    fileName                  );
  fprintf(Pppmfile, "%3d %3d\n", image.width, image.height );
  fprintf(Pppmfile, "255\n"                                );

  while(i < image.width * image.height){

    while(j < 5){
      fprintf(Pppmfile,"%3d %3d %3d ", (image.pixels[i+j]).red, (image.pixels[i+j]).green, (image.pixels[i+j]).blue);
      j++;
    }
...
)
{
  ...
  double start = (startAngleDegrees / 360) * 2 * PI;
  double end   = (endAngleDegrees / 360) * 2 * PI;
  double angle = (end - start) / numberOfSegments;

  Point p1 = makePoint( center.row + radius * sin(start - PI/2)        , center.col - radius * cos(start - PI/2)         );
  Point p2 = makePoint( center.row + radius * sin(start + angle - PI/2), center.col - radius * cos(start + angle - PI/2) );

  while (i < numberOfSegments ){

    drawLine(image, p1, p2, color);
    p1 = p2;
    p2 = makePoint( center.row + radius * sin((i+2)*angle + start - PI/2) , center.col - radius * cos((i+2)*angle + start - PI/2) );
    i++;
...
Boolean drawRectangle ( Image image , Point topLeft , unsigned short int width , unsigned short int height , Color color )
{
  ...
  p1 = topLeft;
  p2 = makePoint( topLeft.row              , topLeft.col + width - 1 );
  p3 = makePoint( topLeft.row + height - 1 , topLeft.col + width - 1 );
  p4 = makePoint( topLeft.row + height - 1 , topLeft.col             );

  drawLine(image, p1, p2, color);
  drawLine(image, p2, p3, color);
  drawLine(image, p3, p4, color);
  drawLine(image, p4, p1, color);
...
Boolean drawRegularConvexPolygon ( Image image , Point center , unsigned short int radius , unsigned short int numberOfSides , Color color )
{
  ...
  double angle = PI * 2 / numberOfSides ;

  Point p1  = makePoint( center.row - radius                       , center.col                                );
  Point p2  = makePoint( center.row + radius * sin( angle - PI/2 ) , center.col - radius * cos( angle - PI/2 ) );
  ...
  while (i < numberOfSides ){
    ...
    p1 = p2;
    p2 = makePoint( center.row + radius * sin( (i+2) * angle - PI / 2 ) , center.col - radius * cos( (i+2) * angle - PI / 2 ) );
...
Boolean drawCharacter ( Image image , Point topLeft , char c , Color color )
{
  ...
  while(j < 13){
    n = characterRasters[i][j];
    while( k < 8 ){
      if( (m & n) == m ){
        point.row = topLeft.row + 13 - j;
        point.col = topLeft.col + (7 - k);
