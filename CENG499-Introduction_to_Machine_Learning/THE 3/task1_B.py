...
clfs = [
    ["0.01", SVC(kernel="poly", C=0.01)],
    ["0.1", SVC(kernel="poly", C=0.1)],
    ["1", SVC(kernel="poly", C=1)],
    ["10", SVC(kernel="poly", C=10)]
]
...
for c_val, clf in clfs:
    ...
    plt.title("C = " + str(c_val) + " - Accuracy: " + str(accuracy))
